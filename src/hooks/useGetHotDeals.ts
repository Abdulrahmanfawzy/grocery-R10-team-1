...
const getHotDeals = async (categoryId: number): Promise<Product[]> => {
  if (categoryId === 0) {
...
const useGetHotDeals = (categoryId: number) => {
  const query = useQuery({
    queryKey: ["home", "hot-deals", categoryId],
    queryFn: () => getHotDeals(categoryId),
    staleTime: 1000 * 60,
  });
